...
button_t *add_button(button_t *buttons, sfVector2f pos, char *txt)
{
    ...
    new->texture = sfTexture_createFromFile("assets/menu/gold_button.png", NULL);
    ...
    sfText_setPosition(new->text, (sfVector2f){pos.x + 10 - (strlen(txt) * 10), pos.y - 5});
...
menu_t *init_menu(void)
{
    ...
    menu->buttons = NULL;
    menu->buttons = add_button(menu->buttons, (sfVector2f){(WIDTH - (menu->rect.width)) / 2, HEIGHT / 2 - 50}, "PLAY");
    add_button(menu->buttons,(sfVector2f){(WIDTH - (menu->rect.width)) / 2, HEIGHT / 2 + 50}, "OPTIONS");
    add_button(menu->buttons,(sfVector2f){(WIDTH - (menu->rect.width)) / 2, HEIGHT / 2 + 150}, "QUIT");
    return menu;
}