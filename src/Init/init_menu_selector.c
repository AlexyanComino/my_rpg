...
static void get_attributes2(char **infos, attributes_t *attributes)
{
    if (infos != NULL) {
        attributes->health = atoi(infos[8]);
...
static void get_attributes(char *txt, select_button_t *new, char **tab)
{
    ...
    get_attributes2(infos, new->attributes);
}
...
select_menu_t *init_select_menu(rpg_t *rpg)
{
    ...
    menu->myclock = NULL;
    menu->text = NULL;
    create_buttons_select(rpg, menu);
    return menu;
}
