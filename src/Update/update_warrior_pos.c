...
static void normal_move(rpg_t *rpg, warrior_t *tmp, warrior_t *enemy)
{
    if (tmp->exclam->is_detecting)
        tmp->state = RUN;
    else if ((warrior_see_enemy(tmp, enemy) && enemy_is_in_base(tmp, enemy)) ||
...
void update_warrior_pos(rpg_t *rpg, warrior_t *tmp, warrior_t *enemy)
{
    ...
    if ((!enemy_is_in_base(tmp, enemy) && warrior_see_enemy(tmp, enemy)) ||
        (enemy_is_in_base(tmp, enemy) && !warrior_see_enemy(tmp, enemy) &&
        !tmp->base->come_back)) {
        tmp->state = IDLE;
        return;
    }
